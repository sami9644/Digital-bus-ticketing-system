...
@login_required
def dashboard():
    return render_template('dashboard.html',userinfo=db.userinfo(session['userid']))
...
def ticket_search():
    ...
    if ticket_data:
        ...
        return "<script>alert('ticket not found!');window.location.href='/dashboard'</script>"
...
def change_picture():
    if request.method == 'POST':
        ...
        profilepic = dp.save(f"static/profilepicture/{session['userid']}.jpg")
        db.uploaddp(f"static/profilepicture/{session['userid']}.jpg",session['userid'])
