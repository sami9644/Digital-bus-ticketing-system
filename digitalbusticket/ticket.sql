...
address TEXT,
addedon TEXT,
FOREIGN KEY(bus) REFERENCES buses(plate_no) ON DELETE CASCADE
);
...
paidby VARCHAR(100),
ticket VARCHAR(100),
FOREIGN KEY(paidby) REFERENCES users(userid) ON DELETE CASCADE,
FOREIGN KEY(ticket) REFERENCES tickets(ticketid) ON DELETE CASCADE
);
