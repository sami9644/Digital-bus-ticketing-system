...
def connection():
	conn = pymysql.connect(user='root',password='Sami@4998',database='busdb',host='127.0.0.1')
	return conn
...
@app.route("/payers/<ticketid>")
@login_required
def payers_list(ticketid):
    with connection() as conn:
        cur = conn.cursor()
        cur.execute("SELECT * FROM payment WHERE ticket = %s",(ticketid,))
        data = cur.fetchall()
    return render_template('payers.html',payers = data,userdata = userinfo)
